...
const FeatureCard: React.FC<FeatureCardProps> = ({ title, imageSrc, mediaOnRight }) => {
  ...
  return (
    <motion.div
      ...
      className={`mx-10 mb-10 min-h-[30vh] sm:min-h-[35vh] md:min-h-[40vh] lg:min-h-[45vh] bg-dark/10 rounded-2xl overflow-hidden`}
    >
      <div className={`flex flex-col ${mediaOnRight ? 'lg:flex-row' : 'lg:flex-row-reverse'}`}>
        ...
        >
          <h3 className="text-charcoal-black text-xl sm:text-2xl md:text-3xl lg:text-4xl font-semibold leading-tight tracking-tight text-center lg:text-left mb-4">
            <span className="text-dark">{title.split(':')[0]}</span>
            <br />
            {title.split(':')[1].trim()}
          </h3>
        </motion.div>
